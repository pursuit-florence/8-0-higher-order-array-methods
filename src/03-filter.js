...
function getSongsBySaib(songs) {
  ...
  const found = songs.filter(song => song.artist === "Saib")
  return found
}
...
function getSongsOverThreeMinutes(songs) {
  ...
  const found = songs.filter(song => song.runtimeInSeconds > 180)
  return found
}
...
const found = songs.filter(song => song.title === song.album)
  return found
